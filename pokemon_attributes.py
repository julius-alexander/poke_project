...
myPokeStatsDB = mysql.connector.connect(
    host="localhost",
    user="root",
    passwd="#Micro$2580852h7",
    database="pokemon"
)

myCursor = myPokeStatsDB.cursor()

# this was a bozo move...
# but temporarily necessary for # 4. of main.py to work correctly
natures_list = [
    'Hardy',
    'Lonely',
    'Brave',
...
class PokeMon:
    ...
    def set_stats(self, evs):
        if self.isLegendary == 'True' and self.level < 60:
            self.level = 60
        self.hp = (floor(
            0.01 * self.level * (2 * self.base_stats[0] + self.ivs[0] + floor(0.25 * evs[0]))) + self.level + 10)
        ...
        self.special_attack = (
                floor(0.01 * self.level * (2 * self.base_stats[3] + self.ivs[3] + floor(0.25 * evs[3]))) + 5)
        self.special_defense = (
                floor(0.01 * self.level * (2 * self.base_stats[4] + self.ivs[4] + floor(0.25 * evs[4]))) + 5)
    ...
    def update_nature_stats(self, new_nature: int):
        self.set_stats(self.evs)  # effectively removes previous nature buff, if applicable
        if new_nature in [0, 6, 12, 18, 24]:  # these natures do not affect stats
            self.nature = new_nature
            return
        if new_nature < 0 or new_nature > 24:  # invalid nature id
            return
        elif 0 <= new_nature <= 24:
            self.nature = new_nature
            self.set_nature_stats()  # re-calculates stats based on nature
